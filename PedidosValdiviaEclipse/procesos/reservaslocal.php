...
<!-- Fin encabezado --> 
	...
	<table border="0px" width="100%">	
		...
		//MUESTRA HISTORIAL DE PEDIDOS CONFIRMADOS
			$re=mysql_query("SELECT restaurantes.Nombre as rest, usuarios.Nombre as solicitante, reservas.ID_Pedido as numeropedido, reservas.Imagen as imagen, reservas.Descripcion as nombre, reservas.Precio as precio, reservas.Cantidad as cantidad, reservas.SubTotal as subtotal, reservas.Estado as estado, reservas.hora_reserva as reserva FROM `reservas` JOIN restaurantes ON reservas.ID_Restaurante=restaurantes.ID_Restaurante JOIN usuarios ON reservas.ID_Cliente=usuarios.ID_Usuario WHERE reservas.ID_Restaurante=".$ID_Cuenta." AND reservas.Estado=0 OR reservas.Estado=1 OR reservas.Estado=2 order by reservas.ID_Pedido DESC");
			$numeropedido=0;
			while ($f=mysql_fetch_array($re)) {
					
					if($numeropedido	!=$f['numeropedido']){
						echo '<tr><td colspan="10"><hr color="#000000"></td></tr>';
    ...
    <br>
    <br>
<center><a class="envio" href="../index.php">Volver</a>&nbsp;<a class="envio" id="limpiar" name="limpiar" href="reservas.php?limpiar?txtRestaurante=<?php echo $_GET['txtRestaurante']; ?>&estado=<?php echo $_GET['estado']; ?>">Limpiar</a></center>
	</section>
</body>
