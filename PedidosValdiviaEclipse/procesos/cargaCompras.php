...
<script>
function cargar(reserva, id, estado) 
{
	var pagina="procesos/reservasconfirmadas.php?txtRestaurante="+id+"&estado="+estado+"&reserva="+reserva;
...
if($_SESSION)
{
  ...
  if($total!=0)
  {
   ...
   <tr>
					<td>
						Costo de envio: $<?php echo $envioVista;?>
					</td>     
				</tr>
                <tr>
					<td> 
  ...
  <tr><td><?php $total=$total+$envio; echo "Total: $".$total; ?></td></tr>
   <tr>
   			<td>
            	<br /><?php if($total!=0){
					?>
				<input type="hidden" id="restaurante" name="restaurante" value="<?php echo $restaurante; ?>" /> <input type="hidden" id="estado" name="estado" value="<?php echo $_GET['estado']; ?>" />
				<a href="javascript:cargar(HoraReserva.value, restaurante.value, estado.value)" class="aceptar">Reservar</a>&nbsp;<?php if($_GET['estado']=='Abierto'){  ?><a href="procesos/pedidosconfirmados.php?txtRestaurante=<?php echo $restaurante; ?>&total=<?php echo $total; ?>" class="aceptar">Comprar</a> <?php }} ?>
            </td>
