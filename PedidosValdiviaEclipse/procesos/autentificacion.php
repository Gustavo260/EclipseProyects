...
//VARIABLES DE AUTENTIFICACION
		...
		//COMPROBACION
		$sql="SELECT usuarios.Nombre as nombre, usuarios.Avatar as avatar, usuarios.Apellido as apellido, usuarios.ID_Usuario, sesiones.ID_Perfil as Perfil, usuarios.Email, usuarios.Clave, usuarios.Domicilio as domicilio, usuarios.Telefono as telefono, perfiles.Estado as Estado FROM usuarios JOIN sesiones ON usuarios.ID_Usuario=sesiones.ID_Usuario JOIN perfiles ON sesiones.ID_Perfil=perfiles.ID_Perfil WHERE Email='".$email."' AND perfiles.Estado=1 AND usuarios.Estado=1";
			...
			if($row=mysql_fetch_array($respuesta))
			{
				...
				if($clavebd==$clave)
				{
					...
					elseif($perfil=="3")
					{
						...
						";	
						$respuesta="Logueado correctamente como restaurante o local";
						header("Location: ../index.php?respuesta=".$respuesta);
					}
				}
